if CLIENT then
	...
	hook.Add("Think", "SWVR.Key.Think", function()
		for key, info in pairs(SWVR.Button.Map) do
			local b = info.b
			info.b = key >= 107 and input.IsMouseDown(key) or input.IsKeyDown(key)

			if info.b ~= b then
				if info.b then hook.Run("PlayerButtonDown", LocalPlayer(), key) else hook.Run("PlayerButtonUp", LocalPlayer(), key) end
			end
		end
	end)
end