...
include("shared.lua")
  ...
  function ENT:ThinkWeapons()
    for _, button in pairs(SWVR.Buttons) do
      for _, tbl in pairs(self.Players or {}) do
        ...
        if not self.Seats[seat]["Weapons"][button] then
          continue
        end
        ...
        if (ply:KeyDown(button)) then
          if (not group.Overheated or not group.CanOverheat and not self:IsCritical()) then
            ...
            group.Overheat = group.Overheat + 10 * FrameTime()
        ...
        if (group.Overheat >= group.OverheatMax) then
          group.Overheated = true
        elseif (group.Overheat <= 0) then
  ...
  function ENT:InitDrive()
    if not self.Hyperdrive then return end
  ...
  function ENT:InitConcussionMissile(data)
    return {
      Type = SWVR.WEAPON_CONCUSSION_MISSILE,
  ...
  -- @param back
  -- @param roll
  function ENT:Setup(health, speed, boost, vertical, accel, back, roll)
    if (speed == nil or not isnumber(speed)) then
      error("Speed must be non-nil and a valid number!")
    end

    self:SetStartHealth(health)
    self:SetShieldHealth(self:GetStartHealth() * 0.5)
    self:SetBack(back)
    self:SetMaxSpeed(speed)
    self:SetMinSpeed(self:GetBack() and (self:GetMaxSpeed() * 0.66) * -1 or 0)
    self:SetVerticalSpeed(vertical)
    self:SetBoostSpeed(boost or speed)
    self:SetAccelSpeed(accel)
    self:SetRoll(roll)
  end
  ...
  function ENT:AddWeaponGroup(name, snd, bullet)
    ...
    local group = {
      ...
      Delay = bullet.Delay,
      Cooldown = CurTime(),
      CanOverheat = bullet.Overheat or true,
  ...
  function ENT:Enter(p)
    if not (self:InFlight() and self.Done) then
      ...
      self:IsTakingOff(true)
      self:SetFlight(true)
      self:SetFreeLook(self.Freelook or false)
  ...
  function ENT:FireWeapons(g)
    ...
    if group.Cooldown > CurTime() then return end
    if group.Overheated then return end
    ...
    for k, v in pairs(self.Weapons) do
      ...
      local angPos = tr.HitPos - v.Ent:GetPos()
      ...
      local bullet = self.WeaponGroups[v.Group].Bullet
      bullet.Attacker = self:GetPilot() or self
      bullet.Src = v.Ent:GetPos()
      bullet.Spread = Vector(spread, spread, spread)
      bullet.Dir = angPos

      v.Ent:FireBullets(bullet)
    end
  ...
  function ENT:FireTorpedo(data, target)
    ...
    torpedo:PrepareTorpedo(self, ent.sound, ent.vel)
  ...
  function ENT:FireProtonTorpedo(w, target)
    local snd = w.Sound or Sound("weapons/n1_cannon.wav")

    local e = ents.Create("proton_torpedo")
    e.Damage = w.Damage or 600
    e.SpriteColor = w.Color or Color(255, 255, 255, 255)
    e.StartSize = w.StartSize or 20
    e.EndSize = w.EndSize or w.StartSize * 0.75
    e.Ion = w.Ion or false
    e:SetPos(w.Pos)
    e:SetAngles(self:GetAngles())
    e:SetCollisionGroup(COLLISION_GROUP_PROJECTILE)
    e:PrepareTorpedo(self, snd, w.Velocity)
  ...
  function ENT:PhysicsSimulate(phys, deltatime)
    ...
    if not (not self.Done and not self.Tractored and (not self.DeactivateInWater or (self.DeactivateInWater and self:WaterLevel() < 3))) then return end
    if (self:InFlight() and IsValid(self:GetPilot())) then
  ...
  function ENT:OnTakeDamage(dmg)
    ...
    if (self:GetShieldHealth() > 0) then
      self:ShieldEffect()
      self:SetShieldHealth(math.max(0, self:GetShieldHealth() - dmg:GetDamage()))
    else
      self:SetCurHealth(math.max(0, self:GetCurHealth() - dmg:GetDamage()))
    end
  ...
  function ENT:OnCritical(default)
